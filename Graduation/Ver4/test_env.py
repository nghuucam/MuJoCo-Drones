...
def test_environment_api():
    print("=" * 80)
    print("🔍 KIỂM TRA TÍNH TƯƠNG THÍCH MÔI TRƯỜNG VER4 (MULTI-INPUT) VỚI SB3 (CHECK_ENV)")
    print("=" * 80)
    ...
    for lvl in range(4):
        ...
        obs_count = len(env.obstacle_data)
        print(f"   - Level {lvl} ({obs_count} cột): Reset OK | Step OK | Reward = {reward:.2f} | Pos = {next_obs['state'][:3].round(2)} | Goal = {next_obs['state'][9:12].round(2)} | APF Hint [α={alpha_h:+.2f}, β={beta_h:+.2f}]")

    env.close()
    print("\n🎉 HOÀN THÀNH TOÀN BỘ KIỂM THỬ MÔI TRƯỜNG VER4!")
