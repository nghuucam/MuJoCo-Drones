...
def plot_moving_average(window_size=50):
    csv_file = find_csv("drone_flight_log_test_D3QN.csv")
    if not os.path.exists(csv_file):
        csv_file = find_csv("drone_flight_log_parallel_D3QN.csv")

    if not os.path.exists(csv_file):
        print(f"❌ Không tìm thấy file flight log CSV cho D3QN!")
        return
    ...
    fig.suptitle(f'Tiến trình Huấn luyện D3QN MuJoCo (Trung bình trượt {window_size} tập)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    out_path = os.path.join(base_dir, "ana4_d3qn_reward_steps.png")
