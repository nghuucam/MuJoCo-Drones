...
def go_up(obs, control, env):
    ...
    while True:
        ...
        if (abs(cur_pos[2] - 1.0) < 0.08 and np.linalg.norm(cur_vel) < 0.20) or (time.time() - start_time > TIMEOUT):
            ...
            sys.stdout.write(f"\r{format_lidar_str(live_sensors)}    \n")
            sys.stdout.flush()
            print(f"🚀 Đã bay lên độ cao {cur_pos[2]:.2f}m an toàn (DQN MuJoCo)!")
            break
...
def main():
    ...
    log_file = os.path.join(current_dir, "drone_flight_log_test_DQN.csv")
    log_entropy_cnn = os.path.join(current_dir, "neutron_array_DQN.csv")
    log_loss = os.path.join(current_dir, "log_loss_DQN.csv")
    eposide_losses = []

    reduce_epison = (max_epsilon - min_epsilon) / (max_eposide0 - 0)
    
    print(f"##################      EPISODE {eposide} (Standard DQN MuJoCo)       ######################### ")
    ...
    while True:
        try:
            ...
            print(f"Q-Values (DQN): {list_action}  -> Chọn: {action_names[action_idx]}")
            ...
            if done:
                ...
                if is_truncated_bool or terminated or (action_count >= MAX_ACTIONS):
                    ...
                    if eposide >= max_eposide:
                        ...
                        print(f"### THỜI GIAN ĐỂ TRAIN BẰNG THUẬT TOÁN DQN (MuJoCo) LÀ {end_time-start_time:.2f} GIÂY ###\n")
                        break
