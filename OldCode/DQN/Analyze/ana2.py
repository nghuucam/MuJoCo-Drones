...
file_name = find_csv("log_loss_DQN.csv")
if not os.path.exists(file_name):
    file_name = find_csv("log_loss_parallel_DQN.csv")

if not os.path.exists(file_name):
    print(f"❌ Không tìm thấy file Loss log CSV cho Standard DQN!")
    sys.exit(1)
...
plt.title('Biểu đồ Tiến trình Giảm Loss (Smooth L1 Loss) qua các tập (Standard DQN MuJoCo)')
...
out_path = os.path.join(base_dir, "ana2_dqn_loss.png")
